...
import { flattenNotificationsPages } from "@features/notification/lib/notifications";
...
type UseNotificationsStreamParams = {
  enabled: boolean;
  userId: number | null;
...
export function useNotificationsStream({
  ...
  useEffect(() => {
    ...
    const connect = async (allowAuthRetry = true) => {
      ...
      const parser = createSseParser((message) => {
        ...
        try {
          onNotificationCreated(JSON.parse(message.data) as NotificationItem);
        } catch (error) {
  ...
  }, [enabled, onNotificationCreated, queryClient, refreshUser, userId]);
}
