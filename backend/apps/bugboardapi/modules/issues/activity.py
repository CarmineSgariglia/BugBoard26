...
from .membership import (
    developer_issue_assignee_users,
    effective_admin_issue_subscription_users,
)
...
class IssueActivityService:
    ...
    def build_attachment_display_name(self, raw_name: str, final_suffix: str) -> str:
    ...
    def save_uploaded_file(self, *, uploaded_file, issue_id: int, base_dir: str):
        content_type, size = validate_issue_attachment(
            uploaded_file,
            max_size_bytes=getattr(
                settings, "BUGBOARD_MAX_ATTACHMENT_FILE_BYTES", 10 * 1024 * 1024
            ),
            max_image_size_bytes=getattr(
                settings, "BUGBOARD_MAX_ATTACHMENT_IMAGE_BYTES", 10 * 1024 * 1024
            ),
        )
        if content_type.startswith("video/"):
            if size > getattr(
                settings, "BUGBOARD_MAX_ATTACHMENT_VIDEO_BYTES", 50 * 1024 * 1024
            ):
            ...
            original_name = self.build_attachment_display_name(
    ...
    def extract_uploaded_files(
        ...
        *,
        ...
        uploaded_files = (
            payload.getlist("file")
            if hasattr(payload, "getlist")
            else payload.get("file")
        )
        ...
        if len(uploaded_files) > max_files:
            if max_files == 1:
                raise ValidationError(
                    {"file": "Exactly one attachment file is required"}
                )
            raise ValidationError(
                {"file": f"Maximum {max_files} files allowed per comment."}
            )

        return uploaded_files

    def delete_media_path(self, path: str) -> None:
        if not path:
            return
        try:
    ...
    def create_attachment_for_event(
        ...
        *,
        ...
        uploaded_files = self.extract_uploaded_files(
            payload,
            required=required,
            max_files=max_files,
        )
        return self._create_attachments_for_event_from_files(event, uploaded_files)

    def schedule_broadcast(self, event: IssueEvent) -> None:
    ...
    def create_event_with_attachments(
        self,
        *,
        issue,
        actor,
        ...
        **extra_fields,
        ...
        normalized_message = self.validate_message(message)
        uploaded_files = self.extract_uploaded_files(
            payload,
        ...
        with transaction.atomic():
            ...
            self._create_attachments_for_event_from_files(event, uploaded_files)
        return event

    def notification_recipients(self, *, issue, actor) -> list[User]:
    ...
    def _create_attachments_for_event_from_files(
        self,
        event: IssueEvent,
        uploaded_files: list,
    ):
        ...
        try:
            with transaction.atomic():
                for uploaded_file in uploaded_files:
                    saved_path, mime_type, size, original_name = (
            ...
            for saved_path in saved_paths:
                self.delete_media_path(saved_path)
            raise

        return attachments


issue_activity_service = IssueActivityService()
