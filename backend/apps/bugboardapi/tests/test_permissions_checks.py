...
from apps.bugboardapi.permissions import (
    ...
    require_admin as public_require_admin,
    require_assignee_or_admin as public_require_assignee_or_admin,
    require_project_access as public_require_project_access,
)
...
from apps.bugboardapi.modules.projects.membership import project_ids_for_user
from apps.bugboardapi.modules.issues.membership import is_developer_issue_assignee
from apps.bugboardapi.modules.projects.membership import is_project_member
from apps.bugboardapi.permissions.scopes import filter_by_project_access, first_by_project_access
...
class PermissionsChecksTests(TestCase):
    ...
    def test_check_admin_denies_non_admin_with_same_message(self):
        with self.assertRaisesMessage(PermissionDenied, "Admin privileges required"):
            require_admin(self.member)

    def test_package_level_permissions_exports_match_public_api(self):
        self.assertIs(public_require_admin, require_admin)
        self.assertIs(public_filter_by_project_access, filter_by_project_access)
        self.assertIs(public_require_project_access, require_project_access)
        self.assertIs(public_require_assignee_or_admin, require_assignee_or_admin)

    def test_check_admin_allows_admin(self):
        require_admin(self.admin)

    def test_user_project_ids_is_scoped_for_non_admin_and_global_for_admin(self):
        member_ids = set(project_ids_for_user(user=self.member))
        admin_ids = set(project_ids_for_user(user=self.admin))
    ...
    def test_project_membership_predicate(self):
        self.assertTrue(is_project_member(user=self.member, project=self.project))
        self.assertFalse(is_project_member(user=self.outsider, project=self.project))
    ...
    def test_ensure_project_access_denies_non_member_with_same_message(self):
        with self.assertRaisesMessage(
            ...
            require_project_access(self.outsider, self.project)
    ...
    def test_created_by_alone_does_not_grant_project_access(self):
        ...
        self.assertEqual(set(project_ids_for_user(user=creator)), set())
        with self.assertRaisesMessage(
            PermissionDenied, "You do not have access to this project"
        ):
            require_project_access(creator, project)

    def test_issue_assignee_predicate(self):
        self.assertTrue(is_developer_issue_assignee(issue=self.issue, user=self.member))
        self.assertFalse(is_developer_issue_assignee(issue=self.issue, user=self.outsider))
    ...
    def test_issue_assignee_predicate_excludes_admin_issue_subscription(self):
        ...
        self.assertFalse(is_developer_issue_assignee(issue=self.issue, user=self.admin))

    def test_project_access_check_denies_user_without_issue_project_access(self):
        with self.assertRaisesMessage(
            PermissionDenied, "You do not have access to this project"
        ):
            require_project_access(self.outsider, self.issue.project)
    ...
    def test_check_assignee_or_admin_denies_non_assignee_with_same_message(self):
        with self.assertRaisesMessage(
            ...
            require_assignee_or_admin(self.outsider, self.issue)

    def test_check_assignee_or_admin_allows_assignee_and_admin(self):
        require_assignee_or_admin(self.member, self.issue)
        require_assignee_or_admin(self.admin, self.issue)
